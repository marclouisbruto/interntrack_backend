...
func LeaveRequestOnDay(c *fiber.Ctx) error {
	...
	// Convert TotalHours from string to seconds
	totalSeconds := 0
	if dtr.TotalHours != "" {
		parts := strings.Split(dtr.TotalHours, ":")
		if len(parts) == 3 {
	...
	// Subtract leave duration in seconds from total
	...
	// Convert updated seconds back to HH:MM:SS
	h := updatedTotal / 3600
	...
	return c.JSON(fiber.Map{
		"message":      "Leave request hours deducted successfully",
